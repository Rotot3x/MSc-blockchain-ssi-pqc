...
class PQCIssuerWorkflow:
    ...
    def __init__(
        self,
        admin_url: str = "http://localhost:9041",
        agent_url: str = "http://localhost:9040",
        ...
        try:
            if method.upper() == "GET":
                async with self.session.get(
                    ...
                    LOGGER.info(
                        f"🟢 API RESPONSE: {response.status} {method.upper()} {url}"
                    )
                    LOGGER.info(
            ...
            LOGGER.error(f"🔴 API ERROR: {method.upper()} {url} - {e}")
            raise
    ...
    async def wait_for_agent_ready(self, timeout: int = 30) -> bool:
        ...
        for i in range(timeout):
            try:
                # Use direct HTTP call to avoid verbose logging during startup
                async with self.session.get(f"{self.admin_url}/status") as response:
                    if response.status == 200:
                        ...
                        if data.get("version") and data.get("conductor"):
    ...
    async def step_1_verify_pqc_plugin(self) -> bool:
        ...
        try:
            ...
            LOGGER.info(
                "✅ PQCrypto_FM plugin assumed to be loaded based on agent startup logs"
            )
            LOGGER.info(
                f"✅ Using PQC algorithms: {self.pqc_signature_algorithm}, {self.pqc_kem_algorithm}"
            )
    ...
    async def step_2a_verify_ledger_connection(self) -> bool:
        ...
        """
        LOGGER.info(
            "🚀 Step 2a: Verifying von-network Hyperledger Indy ledger connection..."
        )
        ...
        try:
            ...
            async with self.session.get(self.genesis_url) as response:
                if response.status == 200:
                    ...
                    LOGGER.error(
                        f"❌ Genesis file not accessible: HTTP {response.status}"
                    )
                    return False
            ...
            LOGGER.info("✅ von-network ledger connection verified!")
    ...
    async def step_3_create_pqc_did(self) -> bool:
        ...
        """
        LOGGER.info(
            "🚀 Step 3: Creating PQC DID using did:indy method with Hyperledger Indy ledger..."
        )
            ...
            try:
                ...
                async with self.session.post(
                    ...
                    if response.status == 200:
                        ...
                        LOGGER.info(
                            "✅ DID registered on von-network Hyperledger Indy ledger!"
                        )
                        LOGGER.info(f"   Registration result: {registration_result}")
                    else:
                        error_text = await response.text()
                        LOGGER.warning(
                            f"⚠️ von-network registration failed: HTTP {response.status} - {error_text}"
                        )
                ...
                try:
                    ...
                    LOGGER.warning(f"⚠️ Could not set as public DID: {public_did_error}")
                    LOGGER.info(
                        "📝 Note: DID created but not set as public, continuing anyway"
                    )
    ...
    async def step_4_create_pqc_schema(self) -> bool:
        ...
        """
        LOGGER.info(
            "🚀 Step 4: Creating Indy schema with PQC metadata on Hyperledger Indy ledger..."
        )
        ...
        try:
            ...
            response = await self.admin_request(
                "POST", "/anoncreds/schema", schema_request
            )
            ...
            LOGGER.info("✅ PQC Schema created on Hyperledger Indy ledger!")
            LOGGER.info(f"   Schema ID: {self.schema_id}")
            LOGGER.info(f"   Schema Name: {schema_request['schema']['name']}")
            LOGGER.info(
                f"   Attributes: {len(schema_request['schema']['attrNames'])} fields"
            )
            LOGGER.info("   Ledger: Hyperledger Indy (von-network)")

            # Verify schema was written to ledger
            try:
                schema_verify = await self.admin_request(
                    "GET", f"/anoncreds/schema/{self.schema_id}"
                )
                LOGGER.info("✅ Schema verification: Found on ledger")
    ...
    async def step_5_create_pqc_credential_definition(self) -> bool:
        ...
        """
        LOGGER.info(
            "🚀 Step 5: Creating Indy credential definition with PQC support on Hyperledger Indy ledger..."
        )
        ...
        try:
            ...
            cred_def_request = {
                ...
                "options": {"support_revocation": True, "revocation_registry_size": 1000},
            }

            # Create credential definition on Indy ledger using AnonCreds
            response = await self.admin_request(
                "POST", "/anoncreds/credential-definition", cred_def_request
            )

            # Extract credential definition ID from response
            if (
                "credential_definition_state" in response
                and "credential_definition_id" in response["credential_definition_state"]
            ):
                self.cred_def_id = response["credential_definition_state"][
                ...
                raise Exception(
                    f"Credential Definition ID not found in response: {response}"
                )

            LOGGER.info(
                "✅ PQC Credential Definition created on Hyperledger Indy ledger!"
            )
            LOGGER.info(f"   Credential Definition ID: {self.cred_def_id}")
            LOGGER.info(f"   PQC Algorithm: {self.pqc_signature_algorithm}")
            LOGGER.info(
                f"   Supports Revocation: {cred_def_request['options']['support_revocation']}"
            )
            LOGGER.info(
                f"   Registry Size: {cred_def_request['options']['revocation_registry_size']}"
            )
            LOGGER.info("   Ledger: Hyperledger Indy (von-network)")

            # Verify credential definition was written to ledger
            try:
                cred_def_verify = await self.admin_request(
                    "GET", f"/anoncreds/credential-definition/{self.cred_def_id}"
                )
                LOGGER.info("✅ Credential Definition verification: Found on ledger")
            except Exception as verify_error:
                LOGGER.warning(
                    f"⚠️ Could not verify credential definition on ledger: {verify_error}"
                )
    ...
    async def step_6_setup_connection_invitation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            LOGGER.info("✅ Connection invitation created successfully!")
    ...
    async def step_8_create_pqc_proof_request_template(self) -> Dict[str, Any]:
        ...
        try:
            ...
            LOGGER.info("✅ PQC proof request template created!")
            LOGGER.info(
                f"   Requested Attributes: {len(proof_request['requested_attributes'])}"
            )
            LOGGER.info(
                f"   Requested Predicates: {len(proof_request['requested_predicates'])}"
            )
            LOGGER.info("   Quantum-Safe Verification: Required")
    ...
    async def step_9_verify_pqc_operations(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                LOGGER.info(
                    f"✅ Public DID: {did_info.get('result', {}).get('did', 'None')}"
                )
            ...
            try:
                ...
                signature_response = await self.admin_request(
                    "POST", "/pqcrypto_fm/sign", sign_request
                )
                LOGGER.info(
                    f"✅ PQC Signature Test: {signature_response.get('success', False)}"
                )
            except Exception as sign_error:
                LOGGER.warning(f"⚠️ PQC signature test skipped: {sign_error}")
                LOGGER.info(
                    f"✅ PQC Plugin: Loaded and configured for {self.pqc_signature_algorithm}"
                )
...
async def main():
    ...
    parser.add_argument(
        "--host", type=str, default="localhost", help="ACA-Py host (default: localhost)"
    )
